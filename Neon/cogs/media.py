...
class Media(commands.Cog):
    ...
    async def add1(self, ctx, channel : discord.TextChannel = None):
        ...
        if channel.id not in [int(i[0]) for i in channels]:
            if len(channels) == 2:
                await ctx.send(f"This server has reached maximum limit of media channels")
                return
            await cursor.execute("INSERT INTO Media(guild_id, channel_id) VALUES(?, ?)", (ctx.guild.id, channel.id))
            await ctx.send(f"Successfully added {channel.mention} to media channels.")
        else:
            await ctx.send(f"That channel is already a media channel")
    ...
    async def remove1(self, ctx, channel : discord.TextChannel = None):
        ...
        if channel.id in ch_:
            ...
            await ctx.send(f"Successfully removed {channel.mention} removed media channels.")
        else:
            await ctx.send(f"This channel isn't in the list of media channels")
        await self.bot.db.commit()
    ...
    async def config1(self, ctx):
        ...
        embed = discord.Embed(title=f"{ctx.guild.name} Media Settings", color=config.color)
        ...
        class ConfirmationView(discord.ui.View):
            ...
            async def callback(self, interaction: discord.Interaction, button: Button):
                if interaction.user.id != ctx.author.id:
                    await interaction.response.send_message(f"Its not your interaction.")
                    return
                await cur.execute("DELETE FROM Media WHERE guild_id = ?", (ctx.guild.id,))
                await interaction.message.delete()
                await ctx.send(f"Successfully reset media channels.")
    ...
    async def add2(self, ctx, role: discord.Role):
        ...
        if role.id not in role_ids:
            if len(role_ids) == 3:
                await ctx.send(f"This server has reached the maximum limit for media ignore.")
                return
            await c.execute("INSERT INTO MediaWhitelist(guild_id, role_id) VALUES(?, ?)", (ctx.guild.id, role.id))
            await ctx.send(f"Successfully added {role.mention} to ignored roles.", allowed_mentions=discord.AllowedMentions.none())
        else:
            await ctx.send(f"That role is already ignored.")
        await self.bot.db.commit()
    ...
    async def remove2(self, ctx, role: discord.Role):
        ...
        if role.id in role_ids:
            ...
            await ctx.send(f"Successfully removed {role.mention} from ignored roles.", allowed_mentions=discord.AllowedMentions.none())
        else:
            await ctx.send(f"That role is not ignored.")
        await self.bot.db.commit()
    ...
    async def config2(self, ctx):
        ...
        embed = discord.Embed(title=f"{ctx.guild.name} Media Roles Settings", color=config.color)
        embed.set_author(name=ctx.author, icon_url=ctx.author.display_avatar.url)
        embed.set_footer(text=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
        embed.timestamp = datetime.datetime.utcnow()
    ...
    async def reset2(self, ctx):
        ...
        class ConfirmationView(discord.ui.View):
            ...
            async def callback(self, interaction: discord.Interaction, button: Button):
                ...
                await interaction.message.delete()
                await ctx.send(f"Successfully reset media ignore roles.")
                await interaction.client.db.commit()
                self.stop()
