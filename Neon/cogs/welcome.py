...
class Welcome(commands.Cog):
    ...
    async def ch_add(self, ctx, channel: discord.TextChannel):
        ...
        if result != []:
            ...
            if len(ch_list) == 3:
                return await ctx.send(f"This server has reached the maximum limit.")
            if channel.id in ch_list:
                return await ctx.send(f"This channel is already in the list of greet channels.")
            
        await cursor.execute("INSERT INTO wlc_ch(guild_id, channel_id) VALUES(?, ?)", (ctx.guild.id, channel.id))
        await ctx.send(f"Successfully added {channel.mention} to greet channels")
    ...
    async def ch_rmv(self, ctx, channel: discord.TextChannel):
        ...
        if result != []:
            ...
            if channel.id not in ch_list:
                return await ctx.send(f"This channel is not in the list of greet channels.")
            
        await cursor.execute("DELETE FROM wlc_ch WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, channel.id))
        await ctx.send(f"Successfully removed {channel.mention} from greet channels")
    ...
    async def message(self, ctx, *, message):
        ...
        if CDB == []:
            await ctx.send(f"Oops, Kindly set your welcome channel first.")
            return
        if result is None:
            sql = ("INSERT INTO Welcome(guild_id, msg, delete_after) VALUES(?, ?, ?)")
            val = (ctx.guild.id, message, 0)
            await ctx.send(f"Successfully set welcome message to `{message}`")
...
""", color=config.color)    
    ...
    async def configuration(self, ctx):
        ...
        da = str(re[1]) + " seconds" if re else 'Disabled'
        if da == '0 seconds':
            da = 'Disabled'
    ...
    async def reset(self, ctx):
        ...
        if data is None:
            await ctx.send(f"There is nothing to reset.")
            return
        ...
        class ButtonView(View):
            ...
            async def callback1(self, interaction: discord.Interaction, button: Button):
                if ctx.author != interaction.user:
                    await interaction.response.send_message(f"Its not your interaction.", ephemeral=True)
                    return
                await cur.execute(f"DELETE FROM Welcome WHERE guild_id = ?", (ctx.guild.id,))
                await interaction.message.delete()
                await ctx.send(f"Successfully reset the welcome system.")
                self.stop()
    ...
    async def test(self, ctx):
        ...
        if result == []:
            await ctx.send(f"Welcome has not been set in this server.")
            return
        
        elif result2 is None and embed_ is None:
            await ctx.send(f"Please setup your welcome message first.")
            return
        ...
        try:
            if result2 is not None:
                ...
                return await ctx.send(f"Please setup your welcome message/embed first.")
        except Exception as e:
            embed = discord.Embed(description=f"Oops, looks like an error occured while sending the message. The reason maybe because you've put wrong variable in the message, please check once and try again.\nIf you are still facing issues please reach [Support Server]({config.Support}) with the following error.\n\n```py\n{e}```", color=config.color)
            embed.set_footer(text=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
            await ctx.send(f"{ctx.author.mention}", embed=embed)
            return
        await ctx.send(f"Messages has been sent.")
    ...
    async def autodel(self, ctx, seconds):
        ...
        try:
            ...
            return await ctx.send(f"Improper input. Ex - `greet autodel 3`")

        if int(seconds) > 10:
            return await ctx.send(f"Seconds must be less then 10 seconds.")
        if int(seconds) < 3 and int(seconds) != 0:
            return await ctx.send(f"Seconds must be `0` or minimum `3`.")
        
        if int(seconds) != 0:
            if al is not None:
                await c.execute("UPDATE Welcome SET delete_after = ? WHERE guild_id = ?", (int(seconds), ctx.guild.id))
                await ctx.send(f"Successfully set `{seconds}` seconds for greet auto deletion.")
            else:
            ...
            if al is not None:
                await c.execute("UPDATE Welcome SET delete_after = ? WHERE guild_id = ?", (int(seconds), ctx.guild.id))
                await ctx.send(f"Successfully disable greet auto deletion.")
            else:
                await c.execute("INSERT INTO Welcome(delete_after, guild_id) VALUES(?,?)", (int(seconds), ctx.guild.id))
                await ctx.send(f"Successfully disable greet auto deletion.")
    ...
    async def toggle(self, ctx):
        ...
        if state is None:
            return await ctx.send(f"Oops, Kindly setup your embed first.")
        
        if state[0] == "disabled":
            await c.execute("UPDATE Embed SET state = ? WHERE guild_id = ?", ("enabled", ctx.guild.id))
            await ctx.send(f"Alright, I've enabled the the embed. Now greet messages will be sent on embeds.")
        else: 
            await c.execute("UPDATE Embed SET state = ? WHERE guild_id = ?", ("disabled", ctx.guild.id))
            await ctx.send(f"Alright, I've disabled the the embed. Now greet messages will be sent normally.")
    ...
    async def ping(self, ctx):
        ...
        if ping[0] == "disabled":
            ...
            await ctx.send(f"Alright, I've enabled the the embed ping. Now the users will get greet pings.")
        else: 
            await c.execute("UPDATE Embed SET ping = ? WHERE guild_id = ?", ("disabled", ctx.guild.id))
            await ctx.send(f"Alright, I've disabled the the embed ping. Now the users will not get pinged.")
    ...
    async def reset2(self, ctx):
        ...
        if embed is None:
            await ctx.send(f"There is no embed to reset.")
            return
        ...
        class ConfirmationView(discord.ui.View):
            ...
            async def ConfirmCallback(self, intr: discord.Interaction, btn: discord.Button):
                if intr.user != ctx.author:
                    await intr.response.send_message(f"Its not your interaction.", ephemeral=True)
                    return
                await intr.message.delete()
                await c.execute(f"DELETE FROM Embed WHERE guild_id = ?", (ctx.guild.id,))
                await ctx.send(f"Successfully reset the greet embed.")
                self.stop()
    ...
    async def add1(self, ctx, role: discord.Role):
        ...
        if roledb is not None:
            if len(result) == 2:
                await ctx.send(f"This server has reached maximum limit. No more autorole for bots can be added.", allowed_mentions=discord.AllowedMentions.none())
                return
            if str(role.id) in [str(i[0]) for i in roledb]:
                await ctx.send(f"That role is already in the list of autorole.", allowed_mentions=discord.AllowedMentions.none())
                return
            if len(result) == 0:
                await cur.execute(f"INSERT INTO Autorole(role_id, guild_id) VALUES(?, ?)", (role.id, ctx.guild.id))
                await ctx.send(f"Successfully added {role.mention} to bot autoroles.", allowed_mentions=discord.AllowedMentions.none())
            if len(result) == 1:
                await cur.execute(f"INSERT INTO Autorole(role_id, guild_id) VALUES(?, ?)", (role.id, ctx.guild.id))
                await ctx.send(f"Successfully added {role.mention} to bot autoroles.", allowed_mentions=discord.AllowedMentions.none())
        await self.bot.db.commit()
    ...
    async def remove1(self, ctx, role: discord.Role):
        ...
        if str(role.id) in [i[0] for i in roledb]:
            if result[0][1] == str(role.id):
                ...
                await ctx.send(f"Successfully removed {role.mention} from bot autoroles.", allowed_mentions=discord.AllowedMentions.none())
            elif result[1][1] == str(role.id):
                await cur.execute(f"DELETE FROM Autorole WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id))
                await ctx.send(f"Successfully removed {role.mention} from bot autoroles.", allowed_mentions=discord.AllowedMentions.none())
        else:
            await ctx.send(f"{role.mention} is not in the list of bot autoroles.", allowed_mentions=discord.AllowedMentions.none())
        await self.bot.db.commit()
    ...
    async def add2(self, ctx, role: discord.Role):
        ...
        if roledb is not None:
            if len(result) == 4:
                ...
                return
            if str(role.id) in [str(i[0]) for i in roledb]:
                await ctx.send(f"That role is already in the list of Human autoroles.", allowed_mentions=discord.AllowedMentions.none())
                return
            if len(result) == 0:
                await cur.execute(f"INSERT INTO AutoroleHuman(role_id, guild_id) VALUES(?, ?)", (role.id, ctx.guild.id))
                await ctx.send(f"Successfully added {role.mention} to human autoroles.", allowed_mentions=discord.AllowedMentions.none())
            if len(result) == 1:
                await cur.execute(f"INSERT INTO AutoroleHuman(role_id, guild_id) VALUES(?, ?)", (role.id, ctx.guild.id))
                await ctx.send(f"Successfully added {role.mention} to human autoroles.", allowed_mentions=discord.AllowedMentions.none())
            if len(result) == 2:
            ...
            await ctx.send(f"That role is already in list of human autoroles.")
        await self.bot.db.commit()
    ...
    async def remove2(self, ctx, role: discord.Role):
        ...
        if str(role.id) in [i[0] for i in roledb]:
            if result[0][1] == str(role.id):
                ...
                await ctx.send(f"Successfully removed {role.mention} from human autoroles.", allowed_mentions=discord.AllowedMentions.none())
            elif result[1][1] == str(role.id):
                await cur.execute(f"DELETE FROM AutoroleHuman WHERE guild_id = ? AND role_id = ?", (ctx.guild.id, role.id))
                await ctx.send(f"Successfully removed {role.mention} from human autoroles.", allowed_mentions=discord.AllowedMentions.none())
            ...
            await ctx.send(f"{role.mention} is not in the list of autoroles.", allowed_mentions=discord.AllowedMentions.none())
        await self.bot.db.commit()
    ...
    async def autorole_config(self, ctx):
        ...
        embed = discord.Embed(title=f"{ctx.guild.name} Autorole Settings", color=config.color)
    ...
    async def Greet_AutoRole_Revoke(self, role: discord.Role):
        c = await self.bot.db.cursor()
        await c.execute("SELECT role_id FROM Autorole WHERE guild_id = ?", (role.guild.id,))
        bot_autorole_raw = await c.fetchall()
        await c.execute("SELECT role_id FROM AutoroleHuman WHERE guild_id = ?", (role.guild.id,))
        autorole_raw = await c.fetchall()

        bot_roles = [int(i[0]) for i in bot_autorole_raw]
        human_roles = [int(i[0]) for i in autorole_raw]

        if role.id in bot_roles:
            await c.execute("DELETE FROM Autorole WHERE role_id = ? AND guild_id = ?", (role.id, role.guild.id))

        if role.id in human_roles:
            await c.execute("DELETE FROM AutoroleHuman WHERE role_id = ? AND guild_id = ?", (role.id, role.guild.id))
