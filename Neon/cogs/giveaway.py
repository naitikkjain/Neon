...
class Giveaway(commands.Cog):
    ...
    async def gstart(self, ctx,
        ...
        if len(g_list) >= 5:
            return await ctx.send(f"You have reached maximum limit for this server.", ephemeral=True)


        if winners == -4:
            await ctx.send(f"Winners wasn't proper. Ex. `&gstart 30m 1w Nitro.`")
            return

        converted = converters.TimeConvert(time)
        if converted/60 >= 40320:
            return await ctx.send(f"Time cannot be more than 28 days.")
        if converted == -1:
            await ctx.send(f"The unit isn't a proper unit.")
            return
        if converted == -2:
            await ctx.send(f"Time must be in numbers.")
            return
        ...
        print(f"[Gstart] Giveaway Started - {ctx.guild.name} ({ctx.guild.id}) - {prize}")
    ...
    async def GiveawayEnd(self):
        ...
        for giveaway in ends_raw:
            if int(current_time) >= round(float(giveaway[0])):
                ...
                if guild is None:
                    await cur.execute("DELETE FROM Giveaway WHERE guild_id = ?", (int(giveaway[1]),))
                    await self.bot.db.commit()
                    continue
                ...
                if channel is not None:
                    try:
                        ...
                        await cur.execute("DELETE FROM Giveaway WHERE guild_id = ?", (int(giveaway[2]),))
                        continue
    ...
    async def gend(self, ctx, message_id = None):
        ...
        if message_id:
            try:
                int(message_id)
            except ValueError: return await ctx.send(f"Invalid message id.")
        ...
        if message_id is not None:
            ...
            if re is None:
                return await ctx.send(f"Giveaway wasn't found.")
            ...
            if re is None:
                return await ctx.send(f"Giveaway was not found.")
    ...
    async def greroll(self, ctx):
        ...
        if not ctx.message.reference:
            await ctx.send(f"Reply with a message to reroll.")
            return

        if ctx.message.reference.resolved.author.id != self.bot.user.id:
            await ctx.send(f"Giveaway was not found.")
            return
        ...
        if re is not None:
            await ctx.send(f"That giveaway is running, please use `gend` command instead to end the giveaway.")
            return
