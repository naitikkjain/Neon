...
class Panel(discord.ui.View):
        ...
        if self.state == 'Image':
            ...
            try:
                ...
                if not urls:
                    ...
                    return await intr.channel.send(f"The image is not an url. Please provide a proper url.", delete_after=5)
                dict = self.embed.to_dict()
            ...
            try:
                ...
                if not urls:
                    try:
                        ...
                        pass
                    await self.message.edit(embed=self.embed)
                    await intr.channel.send(f"The image is not an url. Please provide a proper url.", delete_after=5)
                    return
        ...
        if self.state == 'Color':
            ...
            try:
                ...
                try:
                    ...
                    await intr.channel.send(f"Color must be a hex code.", delete_after=5)
                    return
                dict = self.embed.to_dict()
                dict['color'] = color
    ...
    async def DeleteCallback(self,
        ...
        if self.state == 'Description':
            t = dict.get('title', None)
            if t is None: return await intr.message.delete()
            try: del dict['description']
            except: pass
            embed=self.embed.from_dict(dict)
            self.embed.description = embed.description 
            await self.message.edit(
...
class EmbedBuilder(discord.ui.View):
    ...
    async def SaveEmbedCallback(self,
        ...
        await intr.response.defer()
        if intr.user != self.author:
            await intr.followup.send(f"Its not your interaction.", ephemeral=True)
            return
        ...
        if checker is not None:
            ...
            await intr.followup.send(f"Successfully saved the embed.", ephemeral=True)
        else:
